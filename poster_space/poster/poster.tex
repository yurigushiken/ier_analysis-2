...
\begin{column}{\colwidth}
  ...
  \begin{block}{Introduction}
    ...
    \begin{itemize}
      \item \textbf{Home signers:} deaf children of hearing parents invent gestural communication systems with argument structure, even without linguistic input (Goldin-Meadow, 1985).
  ...
  \begin{alertblock}{Prior Work: Gordon (2003)}
    ...
    \begin{itemize}
      \item In GIVE vs.\ HUG, \textbf{10-month-olds} dishabituated when the toy was removed from GIVE but \textit{not} from HUG, suggesting infants represent the toy as relevant to GIVE's argument structure.
      \item \textbf{8-month-olds} showed a marginal effect for GIVE ($p$=.07); \textbf{6-month-olds} showed no effect.
      \item In a separate SHOW experiment, even \textbf{10-month-olds} failed to dishabituate when the toy was removed, suggesting SHOW may require conceptualization of intention.
    \end{itemize}
...
\begin{column}{\colwidth}
  ...
  \begin{block}{Results: GIVE Condition}
    ...
    \begin{figure}
      ...
      \caption{GIVE: Face$\leftrightarrow$Face transitions increase with age ($\beta$=0.053, $p$=.001). By 10-11 months, infants approached adult levels (20-22\% vs.\ 24.4\%).}
    \end{figure}

    \begin{figure}
      \centering
      \includegraphics[width=0.52\colwidth]{figures/give_motion_tracking.png}
      \caption{GIVE: Toy$\leftrightarrow$Body transitions decrease with age ($\beta$=$-$0.078, $p$=.006).}
    \end{figure}
...
\begin{column}{\colwidth}
  ...
  \begin{alertblock}{GIVE vs.\ SHOW Comparison}
    ...
    Infants approach adult-like gaze patterns for GIVE by 10-11 months but \textit{not} for SHOW, converging with Gordon (2003): GIVE event structure is parsed earlier than SHOW.
